...
test_filename = (
    path + appliance_name + "_test_" + "uk-dale_" + "H" + str(building) + ".csv"
)

chunk = pd.read_csv(
    test_filename, nrows=chunksize, names=["aggregate", appliance_name], header=0
)

# de-normalization
chunk["aggregate"] = chunk["aggregate"] * 822 + 522
chunk[appliance_name] = (
    chunk[appliance_name] * params_appliance[appliance_name]["std"]
